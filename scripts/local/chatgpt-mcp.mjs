...
async function runStart(config, options) {
  ...
  console.log(
    `tunnel ID configured: ${isConfiguredTunnelId(config.tunnelId) ? "yes" : "no"}`,
  );
...
async function runDoctor(config, options) {
  ...
  if (!diagnostics.controlPlaneApiKeyConfigured) {
    console.error(
      "CONTROL_PLANE_API_KEY is required for tunnel-client doctor. Set it in .env, .env.local, or the process environment.",
    );
    return 1;
  }
...
function printDiagnostics(config, diagnostics) {
  console.log(
    `env file (.env): ${diagnostics.envPathPresent ? "present" : "missing"}`,
  );
  console.log(
    `env file (.env.local): ${diagnostics.envLocalPathPresent ? "present" : "missing"}`,
  );
  console.log(`profile: ${config.tunnelProfile}`);
  console.log(`MCP transport: ${config.tunnelMcpTransport}`);
  console.log(`Relay MCP profile: ${config.relayMcpProfile}`);
  if (config.tunnelMcpTransport === "stdio") {
    console.log(`Relay MCP command: ${config.relayMcpStdioCommand}`);
    console.log(
      `local stdio self-test: ${diagnostics.localCheck ?? "not run"}`,
    );
  } else {
    console.log(`Relay MCP URL: ${config.relayMcpUrl}`);
  ...
  console.log(
    `tunnel ID configured: ${diagnostics.tunnelIdConfigured ? "yes" : "no"}`,
  );
  console.log(
    `control-plane key configured: ${diagnostics.controlPlaneApiKeyConfigured ? "yes" : "no"}`,
  );
  console.log(
    `tunnel-client path: ${diagnostics.tunnelClientPath ?? "unresolved"}`,
  );
  console.log(
    `tunnel-client health/admin listener: ${config.tunnelHealthListenAddr}`,
  );
}
...
function requireConfiguredTunnelId(config) {
  if (!isConfiguredTunnelId(config.tunnelId)) {
    throw new ValidationError(
      "TUNNEL_ID is required for init. Set it in .env, .env.local, or the process environment.",
    );
  }
}
...
function requireConfiguredApiKey(config, commandName) {
  if (!isConfiguredApiKey(config.controlPlaneApiKey)) {
    throw new ValidationError(
      `CONTROL_PLANE_API_KEY is required for ${commandName}. Set it in .env, .env.local, or the process environment.`,
    );
  }
}
...
function resolveTunnelClient(config) {
  if (config.tunnelClientPath) {
    ...
    if (!existsSync(explicitPath)) {
      throw new ValidationError(
        `TUNNEL_CLIENT_PATH does not exist: ${explicitPath}`,
      );
    }
    return explicitPath;
  }
  ...
  if (!resolvedPath) {
    throw new ValidationError(
      "Set TUNNEL_CLIENT_PATH in .env, .env.local, or add tunnel-client to PATH.",
    );
  }
...
async function assertRelayReachable(relayMcpUrl) {
  ...
  if (response.statusCode === 405) {
    throw new ValidationError(
      `Relay /mcp returned HTTP 405 at ${relayMcpUrl}. The script must use HTTP POST JSON-RPC for this endpoint.`,
    );
  }

  if (response.statusCode !== 200) {
    throw new ValidationError(
      `Relay /mcp check failed with HTTP ${response.statusCode} at ${relayMcpUrl}.`,
    );
  }
  ...
  try {
    ...
    throw new ValidationError(
      `Relay /mcp returned HTTP 200 at ${relayMcpUrl}, but the body was not valid JSON.`,
    );
  }
  ...
  if (
    ...
    throw new ValidationError(
      `Relay /mcp returned HTTP 200 at ${relayMcpUrl}, but ping did not return a JSON-RPC result.`,
    );
  }
}
...
function postJsonRpcPing(relayMcpUrl) {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    try {
      ...
      rejectPromise(
        new ValidationError(`RELAY_MCP_URL is not a valid URL: ${relayMcpUrl}`),
      );
      return;
    }
    ...
    request.on("timeout", () => {
      request.destroy(
        new ValidationError(
          `Relay /mcp is not reachable at ${relayMcpUrl}. Start the Relay HTTP daemon first, for example: go run ./cmd/relay`,
        ),
      );
    });
    ...
    request.on("error", (error) => {
      ...
      rejectPromise(
        new ValidationError(
          `Relay /mcp check failed at ${relayMcpUrl}: ${error.message}`,
        ),
      );
    });
...
function buildDefaultRelayMcpCommand() {
  ...
  const launcherPath = normalizeCommandPathForTunnel(
    RELAY_MCP_STDIO_LAUNCHER_PATH,
  );
...
function runRelayMcpSelfTest(config) {
  return new Promise((resolvePromise, rejectPromise) => {
    const child = spawn(
      process.execPath,
      [config.relayMcpStdioLauncherPath, "--self-test"],
      {
    ...
    child.on("error", (error) => {
      rejectPromise(
        new ValidationError(
          `Failed to start Relay MCP stdio self-test: ${error.message}`,
        ),
      );
    });
    ...
    child.on("close", (code, signal) => {
      if (stdoutBuffer.trim()) {
        rejectPromise(
          new ValidationError(
            `Relay MCP stdio self-test wrote unexpected stdout: ${stdoutBuffer.trim()}`,
          ),
        );
        return;
      }
      ...
      if (signal) {
        rejectPromise(
          new ValidationError(
            `Relay MCP stdio self-test exited due to signal ${signal}.`,
          ),
        );
        return;
      }
      ...
      if ((code ?? 1) !== 0) {
        ...
        rejectPromise(
          new ValidationError(
            `Relay MCP stdio self-test failed with exit code ${code ?? 1}.${suffix}`,
          ),
        );
        return;
      }
...
function runTunnelClient(command, args, controlPlaneApiKey) {
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("error", (error) => {
      rejectPromise(
        new ValidationError(`Failed to start tunnel-client: ${error.message}`),
      );
    });
    ...
    child.on("close", (code, signal) => {
      if (signal) {
        rejectPromise(
          new ValidationError(`tunnel-client exited due to signal ${signal}.`),
        );
        return;
      }
