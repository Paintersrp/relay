...
- The installer brief names a legacy surgical implementation handoff instructions path under docs/instructions, but this checkout does not contain that file. Current source-controlled contract paths are under `relay-contracts/`; embedded instruction assets are under `internal/instructions/`.
