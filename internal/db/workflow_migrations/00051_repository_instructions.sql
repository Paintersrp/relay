...
-- The package binds the exact repository-instruction basis: every AGENTS.md
-- applicable to the selected Delivery Ticket's inspected source paths,
-- resolved from the exact selected source closure. execution_packages gains
-- the composite digest of the ordered instruction identities, and
-- execution_package_repository_instructions retains the per-instruction
-- identity (repository-relative path and exact SHA-256). Historical package
-- rows predating instruction binding carry the empty digest and fail current
-- revalidation; no runtime path reinterprets them as bound instructions.
PRAGMA foreign_keys=off;

ALTER TABLE execution_packages
ADD COLUMN repository_instructions_sha256 TEXT NOT NULL DEFAULT ''
CHECK (repository_instructions_sha256 = '' OR (length(repository_instructions_sha256) = 64 AND repository_instructions_sha256 NOT GLOB '*[^0-9a-f]*'));

CREATE TABLE execution_package_repository_instructions (
    ...
    UNIQUE (package_row_id, path)
);

CREATE INDEX idx_execution_package_repository_instructions_package
ON execution_package_repository_instructions(package_row_id, sequence, id);

-- +goose StatementBegin
CREATE TRIGGER execution_package_repository_instruction_update_immutable
BEFORE UPDATE ON execution_package_repository_instructions
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'execution package repository instructions are immutable history'); END;
-- +goose StatementEnd
-- +goose StatementBegin
CREATE TRIGGER execution_package_repository_instruction_delete_guard
BEFORE DELETE ON execution_package_repository_instructions
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'execution package repository instructions are retained history'); END;
-- +goose StatementEnd

PRAGMA foreign_keys=on;

-- +goose Down
-- Rebuild execution_packages without the instruction-basis column. Row IDs are
-- preserved so every foreign-key reference remains exact. Entity triggers
-- referencing execution_packages are dropped first and recreated below, and
-- legacy_alter_table keeps the RENAME from re-verifying the remaining
-- audit/run triggers whose bodies reference execution_packages while the old
-- table is transiently absent.
DROP TRIGGER IF EXISTS execution_package_repository_instruction_delete_guard;
DROP TRIGGER IF EXISTS execution_package_repository_instruction_update_immutable;
DROP INDEX IF EXISTS idx_execution_package_repository_instructions_package;
DROP TABLE IF EXISTS execution_package_repository_instructions;
