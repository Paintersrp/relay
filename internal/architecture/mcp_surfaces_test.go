...
func TestMCPRuntimeAndLegacyCatalogRemainIsolated(t *testing.T) {
	root := repoRoot(t)
	for _, relative := range []string{"internal/mcp/route_server.go", "internal/mcp/route_runtime.go", "internal/mcp/route_dispatch.go", "internal/server/mcp_dependencies.go"} {
