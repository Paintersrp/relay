...
const (
	SourceToolListTree = "list_source_tree"
	SourceToolSearch   = "search_source"
	SourceToolReadText = "read_source_text"
)
...
// OwnedSourceToolContract reports whether this package owns the published
// request and response schema for one source tool.
func OwnedSourceToolContract(tool string) bool {
	switch tool {
	case SourceToolListTree, SourceToolSearch, SourceToolReadText:
		return true
	default:
		return false
	}
...
// SourceToolContractSchemas returns the route-independent published input and
