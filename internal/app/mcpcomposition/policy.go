...
func Open(ctx context.Context, vaultRoot string, store *workflowstore.Store, cursorKey []byte, fetcher fileacquisition.FetchOne) (Services, error) {
	...
	return New(store, vaults, publications, cursorKey, fetcher)
}
...
func New(store *workflowstore.Store, vaults *sourcevault.Manager, publications *appoperations.AuthorityPublicationService, cursorKey []byte, fetcher fileacquisition.FetchOne) (Services, error) {
