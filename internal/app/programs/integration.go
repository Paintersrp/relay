...
// recorded accepted isolated-audit decision identity, the carried Ticket
// Shared Design constraints, and the constituent's own validation commands and
...
// integration-eligible with exact recorded facts still current, no selected or
// omitted Ticket dependency to require a missing Program member, no applicable
// Ticket-carried Shared Design constraint to require a missing member, and a
// valid integrated repository state. The Assignment is generated by Relay from
...
// Design constraints are carried from the exact Delivery Ticket bytes recorded
...
func (s *Service) AdmitIntegrationMergeResult(ctx context.Context, workspace, dispatchID, assignmentID string, version int64, input IntegrationMergeResultInput) (IntegrationMergeResult, error) {
	...
	if !sha40.MatchString(strings.ToLower(strings.TrimSpace(input.IntegratedCommit))) || strings.TrimSpace(input.PreservationIdentity) == "" || strings.TrimSpace(input.PreservationIdentity) != input.PreservationIdentity || (input.ConflictEvidence != "" && strings.TrimSpace(input.ConflictEvidence) != input.ConflictEvidence) {
		return IntegrationMergeResult{}, ErrInvalidInput
	}
	...
	err := s.store.WithTx(ctx, func(t *workflowstore.Tx) error {
		...
		row, err := d.ExecContext(ctx, `INSERT INTO program_integration_merge_results(result_id,assignment_row_id,integrated_commit,preservation_identity,conflict_evidence) VALUES(?,(SELECT id FROM program_integration_assignments WHERE assignment_id=?),?,?,?)`, resultID, assignmentID, input.IntegratedCommit, input.PreservationIdentity, sql.NullString{String: input.ConflictEvidence, Valid: input.ConflictEvidence != ""})
		if err != nil {
			return err
		}
		...
		result = IntegrationMergeResult{
			...
			IntegratedCommit: input.IntegratedCommit, PreservationIdentity: input.PreservationIdentity, ConflictEvidence: input.ConflictEvidence,
		}
		for index, outcome := range input.Validations {
...
func (s *Service) ReadIntegrationMergeResult(ctx context.Context, workspace, dispatchID, assignmentID string) (IntegrationMergeResult, error) {
	...
	var result IntegrationMergeResult
	err := s.store.DB().QueryRowContext(ctx, `SELECT r.result_id, r.integrated_commit, r.preservation_identity, COALESCE(r.conflict_evidence,'') FROM program_integration_merge_results r JOIN program_integration_assignments a ON a.id=r.assignment_row_id JOIN program_dispatches d ON d.id=a.dispatch_row_id JOIN feature_workspaces w ON w.id=d.workspace_row_id WHERE a.assignment_id=? AND d.dispatch_id=? AND w.workspace_id=?`, assignmentID, dispatchID, workspace).Scan(&result.ResultID, &result.IntegratedCommit, &result.PreservationIdentity, &result.ConflictEvidence)
	if errors.Is(err, sql.ErrNoRows) {
