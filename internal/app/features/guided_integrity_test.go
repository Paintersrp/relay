...
func TestGuidedIntegrityComposesTypedDiscoveryAuthorityPlanningIdentities(t *testing.T) {
	...
	if _, err := service.CompletePlanningCandidateReview(ctx, CompleteCandidateReviewInput{WorkspaceID: workspace.WorkspaceID, CandidateID: admitted.Candidate.CandidateID, ReviewerIdentity: "auditor", Disposition: PlanningCandidateReviewReadyForApproval, ReviewedBytes: bytes}); err != nil {
		t.Fatal(err)
	}
