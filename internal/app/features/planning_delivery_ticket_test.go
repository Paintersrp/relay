...
func deliveryTicketCandidateBytes(ticketID, featureSlug, repoTarget, commit string) []byte {
	return []byte(`{"schema_version":"2.0","feature_slug":"` + featureSlug + `","ticket_id":"` + ticketID + `","revision":1,"replaces_revision":null,"repo_target":"` + repoTarget + `","branch":"main","base_commit":"` + commit + `","goal":"Produce a deterministic Delivery Ticket.","context":"The candidate is compiled without changing its approved bytes.","scope":{"in_scope":["Compile the candidate."],"out_of_scope":["Mutate candidate bytes."]},"depends_on":[],"required_invariants":["Candidate production binds the exact approved bytes."],"forbidden_behaviors":[],"implementation_obligations":[{"source_area":"internal/app/tickets","obligation":"Produce the approved candidate exactly.","prerequisites":[]}],"proof_obligations":["Prove canonical and rendered identities remain distinct."],"validation_commands":[{"working_directory":"","command":"go test ./internal/app/features","expected":"all tests pass"}],"transition_applicability":"not_required","explicit_deferrals":[],"completion_criteria":["Canonical and rendered identities remain distinct."]}`)
}
