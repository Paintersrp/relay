...
func newWorkflowAuditService(
	...
	if loadPackageEvidence == nil {
		return nil, fmt.Errorf("workflow package evidence loader is required")
	}
...
func (s *WorkflowAuditService) Prepare(ctx context.Context, input PrepareWorkflowAuditInput) (PrepareWorkflowAuditResult, error) {
	...
	if !run.ExecutionPackageRowID.Valid {
		return PrepareWorkflowAuditResult{}, ErrWorkflowAuditPackageRequired
	}
...
func (s *WorkflowAuditService) GetCurrentPacket(ctx context.Context, runID string) (GetWorkflowAuditPacketResult, error) {
	...
	if !run.ExecutionPackageRowID.Valid {
		return GetWorkflowAuditPacketResult{}, ErrWorkflowAuditPackageRequired
	}
...
func (s *WorkflowAuditService) GetCurrentArtifact(ctx context.Context, input GetWorkflowAuditArtifactInput) (GetWorkflowAuditArtifactResult, error) {
	...
	declared, err := resolveWorkflowPackageArtifact(packet, input.ArtifactReference)
	if err != nil {
		return GetWorkflowAuditArtifactResult{}, err
	}
...
func resolveWorkflowPackageArtifact(packet WorkflowPackageAuditPacket, reference string) (WorkflowPackageAuditArtifactReference, error) {
	...
	for _, artifact := range []WorkflowPackageAuditArtifactReference{
		...
		if found != nil {
			return WorkflowPackageAuditArtifactReference{}, ErrWorkflowAuditArtifactReference
		}
...
func (s *WorkflowAuditService) GetStatus(ctx context.Context, runID string) (WorkflowAuditStatus, error) {
	...
	if !run.ExecutionPackageRowID.Valid {
		return WorkflowAuditStatus{}, ErrWorkflowAuditPackageRequired
	}
...
func (s *WorkflowAuditService) RecordDecision(ctx context.Context, input RecordWorkflowAuditDecisionInput) (RecordWorkflowAuditDecisionResult, error) {
	...
	if !run.ExecutionPackageRowID.Valid {
		return RecordWorkflowAuditDecisionResult{}, ErrWorkflowAuditPackageRequired
	}
	...
	if input.AuditPacketID == "" || len(input.PacketSHA256) != 64 || len(input.AuditedCommit) != 40 {
		return RecordWorkflowAuditDecisionResult{}, fmt.Errorf("run_id, audit_packet_id, packet_sha256, and audited_commit are required")
	}
...
// recordPackageDecision depends only on the package packet and its verified
// execution evidence. Package packets are a separate canonical contract.
