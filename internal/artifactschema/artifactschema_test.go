...
func TestPinnedCurrentAuthorityCatalog(t *testing.T) {
	if AuthorityRepository != "Paintersrp/relay-specs" || AuthorityCommit != "c166ca6020e86dad50e68962d733c7c7eb996b5e" {
...
func TestPinnedAuthorityDeliveryTicketSchemaIsExactV20(t *testing.T) {
	...
	if strings.Contains(content, `"shared_design_constraints"`) {
		t.Fatal("pinned v2.0 schema contains Relay-local Ticket authority")
	}
	...
	withRelayLocalField := append(active[:len(active)-1], []byte(`,"shared_design_constraints":[]}`)...)
	valid, err = Validate(KindDeliveryTicket, withRelayLocalField)
	if err != nil || valid {
		t.Fatalf("Relay-local Ticket field valid=%v err=%v", valid, err)
	}
}
