...
func (tx *Tx) SetDiscoveryReopenPointers(ctx context.Context, workspaceID string, revisionRowID, expectedVersion int64) (FeatureWorkspace, error) {
	...
	err := tx.tx.QueryRowContext(ctx, `UPDATE feature_workspaces SET current_discovery_revision_row_id = ?, current_discovery_closure_packet_row_id = NULL, version = version + 1, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE workspace_id = ? AND version = ? RETURNING id, workspace_id, project_row_id, feature_slug, state, version, current_route_state_row_id, current_authority_revision_row_id, discovery_capability_enabled, current_discovery_revision_row_id, current_discovery_closure_packet_row_id, current_delivery_plan_row_id, created_at, updated_at`, revisionRowID, workspaceID, expectedVersion).Scan(&v.ID, &v.WorkspaceID, &v.ProjectRowID, &v.FeatureSlug, &v.State, &v.Version, &v.CurrentRouteStateRowID, &v.CurrentAuthorityRevisionRowID, &v.DiscoveryCapabilityEnabled, &v.CurrentDiscoveryRevisionRowID, &v.CurrentDiscoveryClosurePacketRowID, &v.CurrentDeliveryPlanRowID, &v.CreatedAt, &v.UpdatedAt)
	return v, err
}
