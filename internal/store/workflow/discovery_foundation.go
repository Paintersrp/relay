...
func updateFeatureWorkspace(ctx context.Context, q rowQueryer, set string, value any, workspaceID string, expectedVersion int64) (FeatureWorkspace, error) {
	...
	err := q.QueryRowContext(ctx, `UPDATE feature_workspaces SET `+set+`, version = version + 1, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE workspace_id = ? AND version = ? RETURNING id, workspace_id, project_row_id, feature_slug, state, version, current_route_state_row_id, current_authority_revision_row_id, discovery_capability_enabled, current_discovery_revision_row_id, current_discovery_closure_packet_row_id, created_at, updated_at`, value, workspaceID, expectedVersion).Scan(&workspace.ID, &workspace.WorkspaceID, &workspace.ProjectRowID, &workspace.FeatureSlug, &workspace.State, &workspace.Version, &workspace.CurrentRouteStateRowID, &workspace.CurrentAuthorityRevisionRowID, &workspace.DiscoveryCapabilityEnabled, &workspace.CurrentDiscoveryRevisionRowID, &workspace.CurrentDiscoveryClosurePacketRowID, &workspace.CreatedAt, &workspace.UpdatedAt)
