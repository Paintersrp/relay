...
type PrototypeAuthorization struct {
	ID, ProposalRowID, WorkspaceRowID, WorkspaceVersion, WorkItemRowID, WorkItemVersion, DiscoveryRevisionRowID, SourceClosureRowID, InvocationArtifactRowID, InvocationSizeBytes                                         int64
	AuthorizationID, ProposedRunID, ProposalSHA256, SourceCommit, SourceTree, RepoTarget, BaseCommit, Adapter, Model, VariantsJSON, EvidenceObligationsJSON, LimitsJSON, InvocationSHA256, InvocationMediaType, CreatedAt string
}
...
type PrototypeApproval struct {
	...
	ApprovalID, MutationIdentity, ApprovalRequestSHA256, OperatorConfirmationEvidence, ConsumedIdentity, ConsumedAt, CreatedAt string
}
...
func (tx *Tx) GetPrototypeApprovalByRun(ctx context.Context, runID int64) (PrototypeApproval, error) {
	return scanPrototypeApproval(tx.tx.QueryRowContext(ctx, prototypeApprovalSelect+` WHERE run_row_id = ?`, runID))
}
func (tx *Tx) GetPrototypeApprovalByMutationIdentity(ctx context.Context, identity string) (PrototypeApproval, error) {
	return scanPrototypeApproval(tx.tx.QueryRowContext(ctx, prototypeApprovalSelect+` WHERE mutation_identity = ?`, identity))
}
...
func (tx *Tx) CreatePrototypeAuthorization(ctx context.Context, v PrototypeAuthorization) (PrototypeAuthorization, error) {
	return scanPrototypeAuthorization(tx.tx.QueryRowContext(ctx, `INSERT INTO feature_workspace_prototype_authorizations (authorization_id, proposal_row_id, proposed_run_id, workspace_row_id, workspace_version, work_item_row_id, work_item_version, discovery_revision_row_id, proposal_sha256, source_closure_row_id, source_commit, source_tree, repo_target, base_commit, adapter, model, variants_json, evidence_obligations_json, limits_json, invocation_artifact_row_id, invocation_sha256, invocation_size_bytes, invocation_media_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING `+prototypeAuthorizationColumns, v.AuthorizationID, v.ProposalRowID, v.ProposedRunID, v.WorkspaceRowID, v.WorkspaceVersion, v.WorkItemRowID, v.WorkItemVersion, v.DiscoveryRevisionRowID, v.ProposalSHA256, v.SourceClosureRowID, v.SourceCommit, v.SourceTree, v.RepoTarget, v.BaseCommit, v.Adapter, v.Model, v.VariantsJSON, v.EvidenceObligationsJSON, v.LimitsJSON, v.InvocationArtifactRowID, v.InvocationSHA256, v.InvocationSizeBytes, v.InvocationMediaType))
}
func (tx *Tx) CreatePrototypeRun(ctx context.Context, v PrototypeRun) (PrototypeRun, error) {
	return scanPrototypeRun(tx.tx.QueryRowContext(ctx, `INSERT INTO feature_workspace_prototype_runs (prototype_run_id, authorization_row_id, workspace_row_id, work_item_row_id) VALUES (?, ?, ?, ?) RETURNING `+prototypeRunColumns, v.PrototypeRunID, v.AuthorizationRowID, v.WorkspaceRowID, v.WorkItemRowID))
}
func (tx *Tx) CreatePrototypeApproval(ctx context.Context, v PrototypeApproval) (PrototypeApproval, error) {
	return scanPrototypeApproval(tx.tx.QueryRowContext(ctx, `INSERT INTO feature_workspace_prototype_approvals (approval_id, run_row_id, authorization_row_id, mutation_identity, approval_request_sha256, operator_confirmation_evidence, consumed_identity) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING `+prototypeApprovalColumns, v.ApprovalID, v.RunRowID, v.AuthorizationRowID, v.MutationIdentity, v.ApprovalRequestSHA256, v.OperatorConfirmationEvidence, v.ConsumedIdentity))
}
func (tx *Tx) ApprovePrototypeRun(ctx context.Context, runID string, expectedVersion int64) (PrototypeRun, error) {
	return scanPrototypeRun(tx.tx.QueryRowContext(ctx, `UPDATE feature_workspace_prototype_runs SET lifecycle_state = 'approved', version = version + 1, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE prototype_run_id = ? AND lifecycle_state = 'proposed' AND version = ? RETURNING `+prototypeRunColumns, runID, expectedVersion))
}
...
const prototypeAuthorizationColumns = `id, authorization_id, proposal_row_id, proposed_run_id, workspace_row_id, workspace_version, work_item_row_id, work_item_version, discovery_revision_row_id, proposal_sha256, source_closure_row_id, source_commit, source_tree, repo_target, base_commit, adapter, model, variants_json, evidence_obligations_json, limits_json, invocation_artifact_row_id, invocation_sha256, invocation_size_bytes, invocation_media_type, created_at`
const prototypeRunColumns = `id, prototype_run_id, authorization_row_id, workspace_row_id, work_item_row_id, lifecycle_state, version, process_outcome, cleanup_status, launch_uncertainty_reason, external_process_identity, created_at, updated_at`
const prototypeApprovalColumns = `id, approval_id, run_row_id, authorization_row_id, mutation_identity, approval_request_sha256, operator_confirmation_evidence, consumed_identity, consumed_at, created_at`
const prototypeApprovalSelect = `SELECT ` + prototypeApprovalColumns + ` FROM feature_workspace_prototype_approvals`
...
func getPrototypeAuthorization(ctx context.Context, q rowQueryer, id string) (PrototypeAuthorization, error) {
	return scanPrototypeAuthorization(q.QueryRowContext(ctx, `SELECT `+prototypeAuthorizationColumns+` FROM feature_workspace_prototype_authorizations WHERE authorization_id = ?`, id))
}
func getPrototypeRun(ctx context.Context, q rowQueryer, id string) (PrototypeRun, error) {
	return scanPrototypeRun(q.QueryRowContext(ctx, `SELECT `+prototypeRunColumns+` FROM feature_workspace_prototype_runs WHERE prototype_run_id = ?`, id))
}
func readPrototypeExecution(ctx context.Context, q rowQueryer, workspaceID, runID string) (PrototypeExecutionAggregate, error) {
	...
	approval, err := scanPrototypeApproval(q.QueryRowContext(ctx, prototypeApprovalSelect+` WHERE run_row_id=?`, a.Run.ID))
	if err == nil {
		a.Approval = &approval
...
func scanPrototypeAuthorization(r rowScanner) (v PrototypeAuthorization, e error) {
	e = r.Scan(&v.ID, &v.AuthorizationID, &v.ProposalRowID, &v.ProposedRunID, &v.WorkspaceRowID, &v.WorkspaceVersion, &v.WorkItemRowID, &v.WorkItemVersion, &v.DiscoveryRevisionRowID, &v.ProposalSHA256, &v.SourceClosureRowID, &v.SourceCommit, &v.SourceTree, &v.RepoTarget, &v.BaseCommit, &v.Adapter, &v.Model, &v.VariantsJSON, &v.EvidenceObligationsJSON, &v.LimitsJSON, &v.InvocationArtifactRowID, &v.InvocationSHA256, &v.InvocationSizeBytes, &v.InvocationMediaType, &v.CreatedAt)
	return
}
...
func scanPrototypeApproval(r rowScanner) (v PrototypeApproval, e error) {
	e = r.Scan(&v.ID, &v.ApprovalID, &v.RunRowID, &v.AuthorizationRowID, &v.MutationIdentity, &v.ApprovalRequestSHA256, &v.OperatorConfirmationEvidence, &v.ConsumedIdentity, &v.ConsumedAt, &v.CreatedAt)
	return
}
