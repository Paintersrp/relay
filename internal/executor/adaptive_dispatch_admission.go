...
func NewAdaptiveDispatchAdmissionService(
	...
	return &AdaptiveDispatchAdmissionService{store: store, briefs: briefs, attempts: attempts, runs: runs, assignments: assignments}, nil
}
...
func (s *AdaptiveDispatchAdmissionService) Begin(ctx context.Context, input AdaptiveDispatchAdmissionInput) (AdaptiveDispatchAdmissionResult, error) {
	if s == nil || s.store == nil || s.briefs == nil || s.attempts == nil || s.runs == nil || s.assignments == nil {
	...
	return adaptiveDispatchAdmissionResult(prepared, brief, admitted, validationCmds), nil
}
...
func adaptiveDispatchAdmissionResult(prepared AdaptiveExecutionAttemptResult, brief EffectiveExecutorBriefResult, admitted workflowruns.BeginPreparedAdaptiveExecutionResult, validationCommands []speccompiler.ProjectedValidationCommand) AdaptiveDispatchAdmissionResult {
