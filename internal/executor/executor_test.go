...
func TestDispatchBrief_CreatesChunkEventsDuringStreaming(t *testing.T) {
	s := setupExecutorTestStore(t)
	runID := createExecutorReadyRun(t, s, "approved_for_executor")
	writeExecutorBrief(t, s, runID, "# Brief")
	...
	_, err := DispatchBrief(&DispatchParams{
		...
		RunAgentCmd: func(ctx context.Context, workDir, binary string, args []string, stdin string, timeout time.Duration, callbacks pipeline.AgentCommandStreamCallbacks) pipeline.AgentCommandRunResult {
			chunksSeen = true
			if callbacks.OnStdout != nil {
				callbacks.OnStdout([]byte("hello from executor"))
				callbacks.OnStdout([]byte(`{"type":"tool","part":{"tool":"read","input":{"filePath":"src/main.go"}}}`))
			}
			if callbacks.OnStderr != nil {
				callbacks.OnStderr([]byte("stderr warning"))
			}
			return pipeline.AgentCommandRunResult{ExitCode: 0, Stdout: "done"}
		},
		LaunchAsync: func(fn func()) {
	...
	if !chunksSeen {
		t.Errorf("expected streaming callbacks to be invoked")
	}
	...
	hasStdoutEvent := false
	hasToolEvent := false
	hasStderrEvent := false
	for _, e := range events {
		if strings.Contains(e.Message, "stdout: hello") {
			hasStdoutEvent = true
		}
		if strings.Contains(e.Message, "stdout: tool:read") {
			hasToolEvent = true
		}
		if strings.Contains(e.Message, "stderr: stderr warning") {
			hasStderrEvent = true
		}
	}

	if !hasStdoutEvent {
		t.Errorf("expected event for stdout chunk, got events: %v", events)
	}
	if !hasToolEvent {
		t.Errorf("expected event for tool JSON chunk, got events: %v", events)
	}
	if !hasStderrEvent {
		t.Errorf("expected event for stderr chunk, got events: %v", events)
	}
}
