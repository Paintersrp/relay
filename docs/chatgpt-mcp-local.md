...
`init:all` runs one `runtimes connect` command per role and requires tunnel-client 0.0.9 status JSON to contain the exact fields `alias`, `tunnel_id`, `profile_name`, `health_url`, `health_url_file`, `process_running`, and nested `process.target_kind`/`process.target_value`. The nested target kind must be `server_url`, and `process.target_value` is the exact HTTP(S) MCP endpoint used for binding checks. A zero connect exit alone is not success. `doctor:all` and `status:all` use the same adapter. Tunnel IDs are abbreviated and secrets are redacted from child output. Existing correctly configured runtimes are reused; endpoint or profile drift stops and reconnects that alias before verification. The checked-in sanitized status fixture in `scripts/local/test-fixtures/tunnel-client-0.0.9-status.json` mirrors this production shape.

The aggregate state file is local-only and records desired bindings, aliases changed by the operation, and bounded verified Relay ownership metadata; it never records secrets, executable contents, or health ports. Relay identity adapters use `/proc/<pid>/stat`, `/proc/<pid>/cmdline`, and a symlink read of `/proc/<pid>/exe` on Linux; controlled `ps` output with a C-locale start identity on macOS; and CIM/PowerShell JSON on Windows. Before stopping Relay, PID liveness, start identity, executable identity, and command fingerprint or expected arguments must all be verifiable. If identity capture fails immediately after spawning Relay, the new process tree is terminated and temporary ownership state is removed; an untracked process is never preserved. A lock file prevents duplicate aggregate startup and stale lock recovery checks the recorded lock PID. `SIGINT`, `SIGTERM`, startup failure, and `stop:all` perform exhaustive cleanup: every applicable runtime stop is attempted, Relay verification/shutdown is attempted regardless of tunnel failures, aggregate state removal is attempted in finalization, the lock is released, and all cleanup errors are reported together. If a saved Relay PID is stale or reused, `stop:all` preserves the live process, removes stale state, and reports an actionable diagnostic. POSIX process groups and Windows process trees use platform-specific shutdown adapters.
