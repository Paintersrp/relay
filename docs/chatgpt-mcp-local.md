...
The aggregate state file is local-only and records desired bindings, aliases changed by the operation, and bounded verified Relay ownership metadata; it never records secrets, executable contents, or health ports. State writes use a same-directory temporary file, flush, and atomic replacement; malformed or unsupported state fails closed. Relay identity adapters use `/proc/<pid>/stat`, `/proc/<pid>/cmdline`, and a symlink read of `/proc/<pid>/exe` on Linux; separate controlled `ps` queries with a C-locale start identity on macOS; and CIM/PowerShell JSON on Windows. Before stopping Relay, PID liveness, start identity, executable identity, and command fingerprint or expected arguments must all be verifiable. If identity capture fails immediately after spawning Relay, the new process tree is terminated and temporary ownership state is removed only after confirmed exit; an untracked process is never preserved. A lock file records an owner token and process start identity, preventing PID reuse from being mistaken for a live aggregate operation. `SIGINT`, `SIGTERM`, startup failure, and `stop:all` perform exhaustive cleanup: every applicable runtime stop is attempted, Relay verification/shutdown is attempted regardless of tunnel failures, the lock is released, and all cleanup errors are reported together. Failed or cancelled orchestration preserves the prior valid state. POSIX process groups and Windows process trees use platform-specific shutdown adapters.
