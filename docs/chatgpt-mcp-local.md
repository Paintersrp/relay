...
The version-3 aggregate state file is local-only and records desired bindings, retired/residual bindings awaiting cleanup, and bounded verified Relay ownership metadata. Identity values are recursively redacted before persistence; command display and child output use the same key redaction. State writes use a same-directory temporary file, flush, and atomic replacement; version-2, malformed, or unsupported state fails closed and requires an explicit `init:all` migration. Alias changes are reconciled by role key: the persisted alias is confirmed stopped before the replacement is connected. Partial `stop:all` writes retry state containing only unconfirmed runtimes and any still-owned Relay. Relay identity adapters use `/proc/<pid>/stat`, controlled `ps` start identity on macOS, and CIM `CreationDate` on Windows. Aggregate ownership uses an exclusive `<state>.lock` directory, with a flushed and atomically published owner record for diagnostics. Every aggregate acquisition briefly owns an atomic `<state>.lock-recovery` gate, so stale recovery is serialized; the gate rereads and revalidates the primary lock before any stale removal, and no contender publishes the primary lock while the gate is held. Missing, partial, malformed, unreadable, or unsupported lock or recovery-gate metadata is ambiguous and is never removed automatically; stale recovery requires a dead PID or a definitively mismatched observed start identity, and otherwise instructs the operator to confirm no aggregate operation is running before manual removal. Release renames the owned public lock directory to an owner-token-qualified private residue before cleanup. Release failures are reported as command failures; private `.release-<owner-token>` residue is not an active lock and may be removed only after confirming no aggregate operation is running. `SIGINT`, `SIGTERM`, startup failure, and `stop:all` perform exhaustive journaled cleanup and release only the current lock owner. POSIX process groups and Windows process trees use platform-specific shutdown adapters.
