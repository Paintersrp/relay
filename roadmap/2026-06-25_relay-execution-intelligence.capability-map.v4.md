# Relay Execution Intelligence — Endgame Capability Map v4
...
Version status: v4 incorporates the external v3 planning-packet audit and supersedes v3 as the controlling capability map.
...
The five Deep Research reports are represented by the Research Integration Brief v4, Program Roadmap v4, and this Capability Map v4. Future child Plans of Passes should not attach the raw reports by default. The raw reports remain archival evidence for targeted verification, provenance checks, grep/rg, ambiguity resolution, or explicit program amendments.
...
| `execution_intelligence_optimization` | Later-phase calibration and tuning loop that turns telemetry/evaluation outcomes into improved routing, render-profile, context-policy, prompt-quality, and split-recommendation guidance while preserving human approval gates. |
...
| CAP-A06 | Research evidence policy refinement | Raw research reports should not become default planning context once accepted anchors exist. | Default child-plan packets use Roadmap v4, Brief v4, and Capability Map v4; Deep Research reports are archival targeted evidence only. | v2 roadmap still encouraged too much raw-report attachment. | `program_governance` | Child-plan packet guide states reports are non-default and child plans record targeted evidence use only when consulted. | `foundation` |
...
| CAP-C02 | Packet assessment synthesis + deterministic packetization research | Assessment is advisory and must not become executable scope; policy validation must be separate from structural schema validation. | Renderer/compiler exclusion rules and policy-validation checks for assessment body leakage. | No system-level enforcement yet. | `packet_assessment` | Brief/packet validation fails if assessment advice leaks into executable scope, even if the JSON schema is structurally valid. | `planned` |
...
| CAP-E06 | Deterministic packetization + evaluation research | Failure attribution needs boundary visibility, and policy validation should be distinct from structural JSON/schema validation. | Packet validation failure codes distinguishing handoff, compiler, structural schema, and policy-validation failures. | Failure modes not normalized. | `evaluation_calibration` | Validation report emits stable failure codes and identifies whether failure was structural or policy-level. | `planned` |
...
| CAP-F06 | Prompt quality research | Deterministic prompt/brief quality failures should block before dispatch when objective conditions fail. | Prompt quality scorecard attached to brief validation with deterministic hard-block categories. | No pre-dispatch scorecard. | `renderer_brief` | UI displays hard blocks/warnings/ECPT, and deterministic blockers are explainable without numeric calibration. | `planned` |
| CAP-F07 | Prompt quality + evaluation research | Numeric quality thresholds and warning-to-hard-block promotion should be calibrated from Relay evidence. | Calibrated-threshold governance for prompt-quality warnings, ECPT, and promotion from warning to hard block. | No calibration governance for thresholds. | `renderer_brief`, `evaluation_calibration` | Threshold changes cite telemetry/eval evidence and program-policy review. | `deferred_preserved` |
...
| CAP-G04 | Model-provider research | Cost must be config-driven and current; provider billing varies by cache read/write, long-context bands, data-residency/region uplifts, tool calls, grounding/search, and token categories. | Cost profile schema with effective dates and provider-specific billing fields including cache, long-context, region/residency, tool/grounding-call, input/output, and reasoning-token dimensions. | No cost ledger. | `routing_cost` | Cost estimate and actual reconcile for runs; estimates identify provider-specific modifiers rather than assuming one uniform per-token price. | `planned` |
...
| CAP-I10 | Optimizing-prompts + deterministic packetization + evaluation research | Exploration/reasoning quality, including file localization, decomposition quality, and over-prediction, is a first-order determinant of repair quality and is not captured by final pass/fail. | Reasoning/exploration-alignment metrics over safe trace evidence. | No reasoning/exploration-alignment metric exists. | `evaluation_calibration` | Trace eval reports file-localization recall, decomposition quality, and over-prediction rate by task family without retaining hidden chain-of-thought. | `deferred_preserved` |
...
Minimum child-plan groupings expected below are explanatory. The Program Roadmap v4 PLAN-A through PLAN-L decomposition is canonical. A future child plan may combine or split work only through a reviewed roadmap amendment.
...
## 8. v4 confirmatory-review notes
...
## 9. v4 corrections applied

- CAP-A06 is assigned to PLAN-A and counted in the coverage summary.
- `execution_intelligence_optimization` is defined as a first-class track.
- CAP-F07 distinguishes calibrated numeric thresholds from deterministic hard-blocks.
- CAP-I10 preserves reasoning/exploration-alignment metrics as deferred-preserved evaluation work.
- CAP-G04 explicitly covers cache, long-context, region/data-residency, tool/grounding, and reasoning-token cost dimensions.
- CAP-C02 and CAP-E06 clarify that policy validation is separate from structural schema validation.
