...
export function RelayExecutionPackageDetail({ packageId }: { packageId: string }) {
  ...
  const [error, setError] = React.useState<string | null>(null);
  const approval = useMutation({ mutationFn: () => { const value = query.data; if (!value) throw new Error("Package basis is unavailable."); return approveExecutionPackage(value.packageId, { expectedPackageSha256: value.packageSha256, operatorConfirmationEvidence: confirmationEvidence }); }, onSuccess: () => { setError(null); void queryClient.invalidateQueries({ queryKey: packageKeys.detail(packageId) }); }, onError: (value) => setError(report(value)) });
