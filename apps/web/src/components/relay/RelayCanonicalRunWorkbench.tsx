...
function AuditPanel({
  ...
  const [error, setError] = React.useState<string | null>(null);
	...
	const [findingSource, setFindingSource] = React.useState<"implementation" | "governing_package" | "both">("both");
  ...
  const mutation = useMutation({
	...
	const decisionMutation = useMutation({
		mutationFn: () => {
			...
			return recordWorkflowAuditDecision(runId, {
				...
				materialFindings: decision === "needs_revision" ? [{ source: findingSource, summary: findingSummary.trim(), evidence: findingEvidence.trim(), requiredRemediation: requiredRemediation.trim() }] : [],
	...
	const findingRequired = decision === "needs_revision";
    ...
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-[20rem_minmax(0,1fr)]" data-testid="audit-responsive-grid">
      ...
      >
		...
		{!status.decision && packetQuery.data ? <form className="space-y-3 rounded border border-[var(--relay-row-border)] p-3" onSubmit={(event) => { event.preventDefault(); decisionMutation.mutate(); }}>
			...
			{decision === "needs_revision" ? <div className="grid gap-2 rounded border border-warning/30 p-3"><p className="text-xs text-muted-foreground">Revision requests require one complete material finding.</p><div><Label htmlFor="audit-finding-source">Finding source</Label><select id="audit-finding-source" className="mt-1 w-full rounded border bg-background p-2" value={findingSource} onChange={(event) => setFindingSource(event.target.value as typeof findingSource)}><option value="both">Both</option><option value="implementation">Implementation</option><option value="governing_package">Governing package</option></select></div><div><Label htmlFor="audit-finding-summary">Finding summary</Label><Input id="audit-finding-summary" value={findingSummary} onChange={(event) => setFindingSummary(event.target.value)} required /></div><div><Label htmlFor="audit-finding-evidence">Evidence</Label><Textarea id="audit-finding-evidence" value={findingEvidence} onChange={(event) => setFindingEvidence(event.target.value)} required /></div><div><Label htmlFor="audit-required-remediation">Required remediation</Label><Textarea id="audit-required-remediation" value={requiredRemediation} onChange={(event) => setRequiredRemediation(event.target.value)} required /></div></div> : null}
