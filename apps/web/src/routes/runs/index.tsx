...
import { runsListQueryOptions, getActiveStepRoute, formatRunDate, formatRunDateRelative } from '@/features/relay-runs'
            ...
            >
              ...
              <CardContent className="p-4 pt-2">
                ...
                <div className="flex items-center gap-1 mb-3">
                  {(['intake', 'prepare', 'execute', 'audit'] as const).map((step) => {
                  ...
                  <div className="flex items-center gap-2">
                    {run.validationSummary.errors > 0 && (
                      <Badge variant="destructive" className="text-xs">
                        {run.validationSummary.errors} error{run.validationSummary.errors !== 1 ? 's' : ''}
                      </Badge>
                    )}
                    {run.validationSummary.warnings > 0 && (
                      <Badge variant="warning" className="text-xs">
                        {run.validationSummary.warnings} warning{run.validationSummary.warnings !== 1 ? 's' : ''}
                      </Badge>
                    )}
                  </div>
