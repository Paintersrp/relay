...
export async function prepareExecutionPackage(request: PacketAdmissionRequest & { selectionId: string; ticketDesignBrief: PackageArtifactInput; deterministicOperations?: PackageArtifactInput }): Promise<ExecutionPackageDetail> { const path = "/api/execution-packages"; return detail(await requestWorkflowJson<unknown>("POST", path, request), "POST", path); }
