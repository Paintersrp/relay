...
function detail(value: unknown, method: WorkflowHttpMethod, path: string): ExecutionPackageDetail {
  ...
  const ticketDocument = artifact(item.ticketDocument, method, path);
...
export async function prepareExecutionPackage(request: { selectionId: string; deterministicOperations?: PackageArtifactInput }): Promise<ExecutionPackageDetail> { const path = "/api/execution-packages"; return detail(await requestWorkflowJson<unknown>("POST", path, request), "POST", path); }
