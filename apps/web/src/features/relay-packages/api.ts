...
import type { ExecutionPackageDetail, ExecutionPackageRun, MutationLease, MutationLeaseResult, PackageArtifact, PackageArtifactInput } from "./types";
...
function detail(value: unknown, method: WorkflowHttpMethod, path: string): ExecutionPackageDetail {
  ...
  const ticketDesignBrief = artifact(item.ticketDesignBrief, method, path);
  const deterministicOperations = item.deterministicOperations === null || item.deterministicOperations === undefined ? undefined : artifact(item.deterministicOperations, method, path);
  ...
  return {
    ...
    deterministicOperationsSha256: typeof item.deterministicOperationsSha256 === "string" ? item.deterministicOperationsSha256 : undefined,
    deterministicOperationsCoverage: item.deterministicOperationsCoverage === "complete" || item.deterministicOperationsCoverage === "partial" ? item.deterministicOperationsCoverage : undefined,
...
export async function prepareExecutionPackage(request: { selectionId: string; ticketDesignBrief: PackageArtifactInput; deterministicOperations?: PackageArtifactInput }): Promise<ExecutionPackageDetail> { const path = "/api/execution-packages"; return detail(await requestWorkflowJson<unknown>("POST", path, request), "POST", path); }
export async function approveExecutionPackage(packageId: string, request: { expectedPackageSha256: string; operatorConfirmationEvidence: string }): Promise<ExecutionPackageRun> { const path = `/api/execution-packages/${encodeURIComponent(packageId)}/approvals`; const response = record(await requestWorkflowJson<unknown>("POST", path, request), "POST", path, "package approval"); const result = run(response.run, "POST", path); if (typeof response.packageApprovalId === "string") result.packageApprovalId = response.packageApprovalId; return result; }
...
export async function reconcileMutationLease(runId: string, leaseId: string): Promise<MutationLeaseResult> { const path = `/api/runs/${encodeURIComponent(runId)}/mutation-lease/reconcile`; const value = record(await requestWorkflowJson<unknown>("POST", path, { leaseId }), "POST", path, "mutation lease reconciliation"); if (typeof value.released !== "boolean") return malformedWorkflowResponse("POST", path, "released must be a boolean"); return { released: value.released, lease: lease(value, "POST", path) }; }
