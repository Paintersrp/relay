...
import { getFeatureCompletionStatus, getFeatureWorkspace, getGuidedFeatureWorkspace, listProjectFeatureWorkspaces } from "./api";

export const featureWorkspaceKeys = { all: ["feature-workspaces"] as const, detail: (workspaceId: string) => [...featureWorkspaceKeys.all, workspaceId] as const, guided: (workspaceId: string) => [...featureWorkspaceKeys.all, workspaceId, "guided"] as const, completion: (workspaceId: string) => [...featureWorkspaceKeys.all, workspaceId, "completion"] as const, byProject: (projectId: string) => [...featureWorkspaceKeys.all, "by-project", projectId] as const };
...
export function featureWorkspaceGuidedQueryOptions(workspaceId: string) { return queryOptions({ queryKey: featureWorkspaceKeys.guided(workspaceId), queryFn: () => getGuidedFeatureWorkspace(workspaceId), staleTime: 5_000 }); }
